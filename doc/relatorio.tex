...
\section{Introdução}
Este trabalho consiste na criação de uma \textit{word ladder} a partir de um ficheiro de texto, que contém uma lista de palavras. A \textit{word ladder} é uma sequência de palavras, em que cada palavra difere da anterior por apenas uma letra. Por exemplo, \textit{tudo, todo, nodo, nado, nada} é uma \textit{word ladder} entre as palavras \textit{tudo} e \textit{nada}.

\section{Estrutura de dados}
Para armazenar as palavras obtidas a partir do ficheiro de texto, bem como as suas interligações, foi utilizada uma \textit{hash table}, pelo facto de esta possuir uma complexidade computacional \begin{math}O(1)\end{math} para a inserção e remoção de elementos. Esta complexidade computacional é importante, uma vez que, caso esta fosse \begin{math}O(n)\end{math} e existisse um número avultado de palavras, a \textit{word ladder} poderia demorar algum tempo a ser gerada.

A \textit{hash table} é composta por um array de \textit{buckets}, que contêm as palavras e as suas interligações. Cada \textit{bucket} é constituido por uma \textit{linked list}. A \textit{linked list} contém \textit{nodes}, onde, no caso de haver colisões, existirá mais do que um. Cada \textit{node} contém uma palavra e um apontador para o próximo \textit{node} da \textit{linked list}. A figura \ref{fig:hash_table} mostra a estrutura da \textit{hash table}.
...
Para verificar se existem \textit{memory leaks} no programa criado, foi utilizado o \foreign{Valgrind}. Este programa é um conjunto de ferramentas para a deteção de erros de \foreign{memory} e de \foreign{threading}. Para o efeito, foi utilizado o \foreign{Memcheck}, que é uma ferramenta para deteção de erros de memória.

Obteve-se o seguinte resultado, para o programa \lstinline|solution_word_ladder|, com argumento \lstinline|wordlist-four-letters.txt|:
